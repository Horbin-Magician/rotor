<template>
  <main
    ...
    @wheel="handleWheel"
  >
    <PinCanvas ref="canvasRef" :zoom-scale="zoomScale" />

    <PinTips :visible="show_tips" :tips="tips" />
    ...
    <div v-if="canResizeSelection" class="resize-handles">
      <div
        ...
        @mousedown.stop.prevent="startSelectionResizeFromHandle($event, resizeHandleEdges.topLeft)"
      />
      <div
        class="resize-handle resize-top-right"
        @mousedown.stop.prevent="startSelectionResizeFromHandle($event, resizeHandleEdges.topRight)"
      />
      <div
        class="resize-handle resize-bottom-left"
        @mousedown.stop.prevent="
          startSelectionResizeFromHandle($event, resizeHandleEdges.bottomLeft)
        "
      />
      <div
        class="resize-handle resize-bottom-right"
        @mousedown.stop.prevent="
          startSelectionResizeFromHandle($event, resizeHandleEdges.bottomRight)
        "
      />
      <div
        class="resize-handle resize-left"
        @mousedown.stop.prevent="startSelectionResizeFromHandle($event, resizeHandleEdges.left)"
      />
      <div
        class="resize-handle resize-right"
        @mousedown.stop.prevent="startSelectionResizeFromHandle($event, resizeHandleEdges.right)"
      />
      <div
        class="resize-handle resize-top"
        @mousedown.stop.prevent="startSelectionResizeFromHandle($event, resizeHandleEdges.top)"
      />
      <div
        class="resize-handle resize-bottom"
        @mousedown.stop.prevent="startSelectionResizeFromHandle($event, resizeHandleEdges.bottom)"
      />
    </div>
  </main>
...
const parsedPinId = Number.parseInt(appWindow.label.split('-')[1] ?? '', 10)
const pin_id = Number.isNaN(parsedPinId) ? -1 : parsedPinId
...
async function syncCurrentScaleFactor(scaleFactor?: number) {
  const nextScaleFactor =
    scaleFactor ?? (await appWindow.scaleFactor().catch(() => window.devicePixelRatio || 1))
...
function getResizeWindowState(crop: CropRegion, startState: ResizeStartState): ResizeWindowState {
  ...
  const shouldMove = startState.edges.horizontal === 'left' || startState.edges.vertical === 'top'
...
async function flushResizeWindowApply() {
  ...
  if (nextState.position) {
    resizeTasks.push(
      appWindow.setPosition(new PhysicalPosition(nextState.position.x, nextState.position.y)),
    )
  }
