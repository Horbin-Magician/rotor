...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    fs::create_dir_all(native.join("pins/images"))?;
    fs::write(
        native.join("config.toml"),
        "theme = '1'\nfuture_key = 'keep me'\nquick_actions = '[]'\n",
    )?;
    fs::write(
        native.join("pins/record.toml"),
        "future_top = 'keep me'\n[future_section]\nvalue = 'keep me'\n",
    )?;
    ...
    let native_text = fs::read_to_string(native.join("pins/record.toml"))?;
    let document: toml::Value = toml::from_str(&native_text)?;
    assert_eq!(document["future_top"].as_str(), Some("keep me"));
    assert_eq!(
        document["future_section"]["value"].as_str(),
        Some("keep me")
    );
    assert_eq!(
        image::open(native.join(format!("pins/images/{id}.png")))?.to_rgba8(),
        image
    );
    assert_eq!(
        fs::read_to_string(native.join("pins/record.toml"))?,
        native_text
    );
