...
pub(super) fn screen_cursor_position(_scale: f32) -> Option<(f64, f64)> {
    let (x, y) = crate::sys_util::get_cursor_position().ok()?;
    Some((x as f64, y as f64))
}

/// Window levels are an AppKit concept; GPUI's popup style already stays above IME windows.
pub(super) fn configure_text_entry_panel(_handle: WindowHandle<'_>) -> Result<(), String> {
    Ok(())
}

pub(super) fn enable_pin_taskbar(handle: WindowHandle<'_>) -> Result<(), String> {
    use windows::Win32::{
...
/// `enable_pin_taskbar` already adds WS_MINIMIZEBOX; there is no separate style mask step.
pub(super) fn enable_pin_minimization(_handle: WindowHandle<'_>) -> Result<(), String> {
    Ok(())
}
...
pub(super) fn disable_window_animation(handle: WindowHandle<'_>) -> Result<(), String> {
...
pub(super) fn use_small_window_corners(handle: WindowHandle<'_>) -> Result<(), String> {
...
pub(super) fn activate_window_in_place(handle: WindowHandle<'_>) -> Result<(), String> {
...
pub(super) fn pointer_capture(handle: WindowHandle<'_>, capture: bool) -> Result<(), String> {
...
pub(super) fn client_origin(handle: WindowHandle<'_>) -> Result<(i32, i32), String> {
...
pub(super) fn fit_client_bounds(
    handle: WindowHandle<'_>,
    x: i32,
    y: i32,
    width: u32,
