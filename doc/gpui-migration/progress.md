...
2026-09-08 最新指令：跳过全部可视化测试，继续 Windows 工程迁移；macOS 暂缓。已将六个共享 crate 和资源迁至根 `crates/` / `assets/`，同步两壳构建入口，通过 workspace 174 项自动测试与原生严格检查，详见[根目录迁移记录](evidence/windows-root-layout.md)。以下 UI 首要门槛为历史执行顺序，现不再阻挡工程工作。

当前未完成项集中见 [剩余任务汇总](remaining-tasks.md)（2026-09-08）。本文件按实施顺序追加，早期“下一步/尚未实现”需结合后续记录阅读。

当前首要任务（2026-09-08 用户要求）：先参考旧 UI 完成原生 UI 现代化美化，美观度和实用性均不低于旧 UI，再推进其余剩余任务。设置、搜索、翻译、截图和贴图/OCR 的第一批改动已实施，取得部分 Windows 设置窗口截图并修正信息密度；UI-01–UI-05 尚待完整旧新对照验收，详见 [UI 证据](evidence/ui-modernization.md)。

用户最新调整：暂缓 macOS 相关任务，本轮专注 Windows；不以 Windows 结果代替 macOS 验收。
...
## P8 Windows 文件切换与首次启动回退

- NSIS 在安装目录旁完整暂存文件，拒绝覆盖不明非空目录，再将旧安装保留为独立目录后切换新包；解包/重命名失败保留旧文件，目标和恢复路径限定为注册安装目录及其同级目录。
- 有旧程序时，完成页启动新版本会带首次安装检查标志；启动失败可调用新卸载器的专用回退入口，等待失败进程退出，恢复旧程序、保留失败新包并重新打开旧版。常规卸载段不会在回退路径执行，用户资料不回写。
- 新增内部产品/版本核验、路径边界与回退入口的 native clippy 检查通过；NSIS 回退配方已用纯编译夹具验证。没有实际安装、重命名系统目录、触发 UAC 或执行回退，真实失败注入仍是 G8 门槛。

## P8 构建来源收据

- 模式/版本/文件哈希之外，快照记录源码、Cargo 输入、资源和安装配方摘要；构建前后校验输入未变，暂存/打包拒绝相同版本号下的陈旧二进制。文本行尾规范化，供双平台候选合并核验。
- Git 仅用于只读列举文件，覆盖未提交新增文件并忽略生成输出；缺失/过期来源收据测试及 xtask clippy 通过。后续最终候选会在代码冻结后重新构建。

## P7/P9 导出时序收敛

- 保存/复制快捷键在贴图创建或画布渲染期间进入单项等待队列；就绪后继续，渲染失败取消并显示错误，不会静默导出旧帧。等待期间冻结编辑，关闭操作可取消待导出请求。
- 导出前按当前窗口更新画布；打开保存对话框前持有最终 RGBA 快照，随后 DPI/重绘不会替换这次保存的图像。取消对话框会清除准备提示。
- ui 12 项既有回归测试和 native/ui clippy 通过；实际快捷键连续输入、保存对话框与跨屏 DPI 交互仍待新一轮桌面验收。

## 2026-09-08 候选与未通过门槛

- 已冻结原生代码并生成两个最终 Windows 候选，125 项 native/core 测试通过，workspace/格式/clippy/两平台依赖图检查通过。候选清单、源码摘要与 SHA-256 已只读核验。
- 最后批量启动候选做诊断的命令被自动审批审查以 blocked by policy 拒绝，没有执行或绕过；此前 Esc 也已停止本轮桌面交互。Mac 编译/实机、真实安装升级恢复及性能/交互矩阵仍未完成。
- 详见 [候选验收记录](candidate-status.md)。G8/G9 保持未通过，旧壳保留；没有推送、发布、使用真实签名密钥或改动真实用户安装。

## 2026-09-09 分阶段 Windows 续作

- `e968779` 清理 91 个旧文件，保留旧公钥兼容 fixture 与历史源码；`ec802cd` 选择递增候选 `2.6.1-rc.1`。
- `e992a80` 通过真实 socket 验证下载故障与重试；`9fa21cf` 取得开发身份递增安装、自启动卸载清理和限定待机 CPU 数据。
- `89311a8` 交付独立 Windows 恢复启动器；真实子进程监控和独立安装的装载失败回退通过。最终双身份包通过资源/身份/库存检查，只有开发身份执行实际安装。
- 175 项 workspace 测试及工程检查通过；恢复补充测试、正式身份 common 14 项测试通过。一次旧安装器暂存目录切换失败和新目录重试通过分别保留，未掩盖失败。
- 旧壳物理清理不再受阻；远端登录/签名、隔离正式系统和完整性能/故障矩阵继续未完成。视觉测试跳过，macOS 暂缓，G8/G9 保持未通过。完整证据见 [Windows 非可视化续作](evidence/windows-nonvisual-rc1.md)。
