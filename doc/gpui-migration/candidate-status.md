...
`7d2b59f` 修复新目录首装及静默错误反馈，实际通过开发身份的无窗口安装/同版本替换/失败保留/卸载。安装期间只执行身份和资源诊断，已清理开发注册项及快捷方式，测试数据和备份保留；见[安装证据](evidence/windows-silent-install.md)。未签名或发布，不能代替正式身份及递增更新验证。
